...
export default function PublicOrder({ token }: { token: string }) {
  ...
  useEffect(() => {
    let active = true;
    api<PublicSummary>(path).then(value => { if (active) setData(value); }).catch(e => { if (active) setError(e.message); });
    return () => { active = false; };
  }, [path]);
  async function decide() {
    ...
    try {
      ...
      setDecision(null);
    } catch (e) { setError(e instanceof Error ? e.message : 'Não foi possível registrar a decisão.'); }
    finally { setBusy(false); }
  }
  return <main className="detail-panel" style={{ maxWidth: 960, margin: '24px auto', padding: 24 }}>
    <Brand />
    {error && <p role="alert" className="error">{error}</p>}
    {!data ? <p>{error ? 'Solicite um link válido à oficina.' : 'Carregando acompanhamento…'}</p> : <>
