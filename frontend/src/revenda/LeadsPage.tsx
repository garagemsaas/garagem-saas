...
export default function LeadsPage({ userId }: { userId: string }) {
  ...
  return <><div className="page-heading"><div><h1>Leads</h1><p>Atendimento comercial ligado aos clientes da empresa.</p></div><button className="primary" onClick={() => setPanel('novo')}>Novo lead</button></div>
    ...
    {panel && <Drawer title={panel === 'novo' ? 'Novo lead' : panel === 'status' ? 'Atualizar atendimento' : 'Histórico do lead'} close={close}>
