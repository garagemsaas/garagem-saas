...
export default function LeadsPage({ userId, empresa }: { userId: string; empresa: string }) {
  ...
  return <><div className="page-heading"><div><h1>Interessados</h1><p>Quem procurou a loja e em que ponto está a conversa.</p></div><button className="primary" onClick={() => setPanel('novo')}>Novo interessado</button></div>
    ...
    <Collection<Lead> key={`${version}-${status}`} path={`/revenda/leads?status=${status}`} render={l => <><span className="dealer-status">{rotulo(l.status)}</span><h2>{l.clienteNome}</h2><p>{l.telefone} · {l.email}</p><p>{l.veiculoDescricao || 'Interesse ainda não definido'} · {rotulo(l.origem)}</p><p>{l.observacoes}</p><div className="dealer-actions"><WhatsApp telefone={l.telefone} mensagem={convite(empresa, l.veiculoDescricao ? `Sobre o ${l.veiculoDescricao}: seguimos à disposição.` : 'Seguimos à disposição para a sua procura.')} />{next[l.status] && <button onClick={() => { setSelected(l); setPanel('status'); }}>Atualizar atendimento</button>}<button onClick={() => { setSelected(l); setPanel('historico'); }}>Histórico</button></div></>} />
