 ...
 */
export default function ReservasPage({ abrirCarro }: { abrirCarro: (estoqueId: string) => void }) {
  ...
  return <><h1>Reservas</h1><p>Para reservar, cancelar ou vender, abra o carro.</p>
    ...
    <Collection<Reserva> key={status} path={`/revenda/reservas?status=${status}`} empty="Nenhuma reserva registrada."
      render={r => <>
        ...
        <div className="dealer-actions"><button onClick={() => abrirCarro(r.estoqueId)}>Abrir carro</button></div>
      </>} />
  </>;
}
